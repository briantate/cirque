...
# pw_activate.
PROJECT_ROOT="$(dirname "$_PW_BOOTSTRAP_PATH")"
...
export PW_BRANDING_BANNER="$PROJECT_ROOT/tools/pigweed/banner.txt"
...
if [ "$(basename "$_PW_BOOTSTRAP_PATH")" = "bootstrap.sh" ] || \
  ...
  pw_bootstrap --shell-file "$SETUP_SH" \
               ...
               --config-file "$PW_PROJECT_ROOT/tools/pigweed/env_setup.json"
